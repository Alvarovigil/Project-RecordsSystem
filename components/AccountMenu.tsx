...
export default function AccountMenu() {
  const { available, loading, user, profile, signInWithGoogle } = useSession();
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-[220px] border border-line-overlay bg-surface-overlay py-1 shadow-popover">
          <div className="border-b border-paper/10 px-3 pb-2 pt-1.5">
            ...
            <div className="mono truncate text-[11px] text-paper/40">
              @{profile?.username}
            </div>
          </div>
          {/* Only what is not already in the bar two centimetres to the left.
              ...
              a second navigation that could disagree with the first. Signing
              out went the same way: it lives at the bottom of Ajustes, which
              is one press further and the right distance for the only action
